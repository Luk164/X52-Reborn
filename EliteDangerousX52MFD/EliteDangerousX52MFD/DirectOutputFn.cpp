...
{
	cout << "Loading DirectOutput library... ";
...
HRESULT DirectOutputFn::Initialize(const wchar_t * wszPluginName)
{
	...
	auto initDOFn = Pfn_DirectOutput_Initialize(GetProcAddress(dll, "DirectOutput_Initialize"));
...
HRESULT DirectOutputFn::Deinitialize()
{
	...
	auto deInitfunc = Pfn_DirectOutput_Deinitialize(GetProcAddress(dll, "DirectOutput_Deinitialize"));
	hr = deInitfunc();
...
void DirectOutputFn::RegisterDevice()
{
	...
	auto fnRegisterDeviceCallback = Pfn_DirectOutput_RegisterDeviceCallback(GetProcAddress(dll, "DirectOutput_RegisterDeviceCallback"));
	...
	auto fnEnumerate = Pfn_DirectOutput_Enumerate(GetProcAddress(dll, "DirectOutput_Enumerate"));
...
{
	cout << "Getting device... ";
	auto fnGetDeviceType = Pfn_DirectOutput_GetDeviceType(GetProcAddress(dll, "DirectOutput_GetDeviceType"));
	GUID typeGUID = { 0 };
	if (m_devices.empty())
	{
...
HRESULT DirectOutputFn::setDeviceProfile(wchar_t* filepath)
{
	...
	auto fnSetProfile = Pfn_DirectOutput_SetProfile(GetProcAddress(dll, "DirectOutput_SetProfile"));
...
{
	cout << "Adding page... ";
	auto fnSetPage = Pfn_DirectOutput_AddPage(GetProcAddress(dll, "DirectOutput_AddPage"));
	void * hdevice = m_devices[0];
...
{
	//cout << "Setting string... ";
	auto fnSetString = Pfn_DirectOutput_SetString(GetProcAddress(dll, "DirectOutput_SetString"));
	void * hDevice = m_devices[0];
...
HRESULT DirectOutputFn::registerSoftBtnCallback()
{
	...
	auto fnRegSoftBtn = Pfn_DirectOutput_RegisterSoftButtonCallback(GetProcAddress(dll, "DirectOutput_RegisterSoftButtonCallback"));
...
HRESULT DirectOutputFn::registerPageCallback()
{
	...
	auto fnRegPageCallback = Pfn_DirectOutput_RegisterPageCallback(GetProcAddress(dll, "DirectOutput_RegisterPageCallback"));
...
HRESULT DirectOutputFn::unRegisterPageCallback()
{
	...
	const auto fnRegPageCallback = Pfn_DirectOutput_RegisterPageCallback(GetProcAddress(dll, "DirectOutput_RegisterPageCallback"));
	hr = fnRegPageCallback(m_devices[0], nullptr, nullptr);
	return hr;
}
...
{
	auto pThis = static_cast<DirectOutputFn*>(pCtxt);
...
void __stdcall DirectOutputFn::OnDeviceChanged(void * hDevice, bool bAdded, void * pCtxt)
{
	...
	else
	{
		...
		for (auto it = pThis->m_devices.begin(); it != pThis->m_devices.end(); ++it)
		{
			if (*it == hDevice)
			{
...
void DirectOutputFn::updatePage(int pageNumber)
{
	...
	case 1:
		...
		{
			const int vectorSize = jsonDataClass.pg1.cmdrPage1Info.size();
			if (vectorSize == 1)
			{
